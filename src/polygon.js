...
Polygon.prototype = {
    ...
    scale: function (k, point) {
        ...
        this.vertices.each(function (node) {
            ...
            P[0] + k * (E[0] - P[0]), P[1] + k * (E[1] - P[1]), (P[2] || 0) + k * (E[2] - (P[2] || 0))]);
        });
    ...
    area: function () {
        if (this.isTriangle()) {
            ...
            B = B.data.elements;
            C = C.data.elements;
            return 0.5 * Vector.create([(A[1] - B[1]) * (C[2] - B[2]) - (A[2] - B[2]) * (C[1] - B[1]), (A[2] - B[2]) * (C[0] - B[0]) - (A[0] - B[0]) * (C[2] - B[2]), (A[0] - B[0]) * (C[1] - B[1]) - (A[1] - B[1]) * (C[0] - B[0])]).modulus();
        } else {
    ...
    triangulateByEarClipping: function () {
        var poly = this.dup(),
            triangles = [],
            success, convexNode, mainNode, trig;
